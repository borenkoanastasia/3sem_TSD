...
int tw_array_pop(tw_array *turn, adress_stack_node_t **head)
{
	...
	add_adresses(head, (long int)turn->pin);
	turn->pin--;
	return element;
}
void tw_array_free(tw_array *turn, adress_stack_node_t **head)
{
	for (int *cur = turn->pout; cur < turn->pin; cur++)
	{
		add_adresses(head, (long int)cur);
	}
	free(turn->pout);
}

void tw_s_array_free(tw_array *turn)
{
	free(turn->pout);
}

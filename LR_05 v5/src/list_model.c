...
{
	list_node_t *last = (*t1)->pin;
	request_t *request;
	//char d;
	//output_turn(**t1);

	create_request(&request, a1->current_request.id, sys_time);

	//debug_print(DEBUG, "\n\n"); 
	list_turn_push(t1, request);
	if ((*t1)->pout == NULL)
		(*t1)->pout = last;
	(*t1)->len++;
	if (last != NULL)
...
void list_remove_from_turn_to_automat(list_turn_t **t1, automat_t *a1, my_time_t sys_time, adress_stack_node_t **adress_stack)
{
	request_t request;
	//output_turn(**t1);
	
	// Удаление заявки из очереди
	list_turn_pop(t1, &request, adress_stack);
	(*t1)->len--;
	if ((*t1)->pout == NULL)
		(*t1)->pin = NULL;
...
void list_beat(list_turn_t **t1, automat_t *a1, my_time_t sys_time, adress_stack_node_t **adress_stack)
{
	//debug_print(DEBUG, "\nSTART BEAT\n");
	//output_automat(*a1);
	if (a1->current_request.id != -1)
	...
	if ((*t1)->len != 0)
	{
		...
		list_remove_from_turn_to_automat(t1, a1, sys_time, adress_stack);
	}
	...
	int condition)
{
	...
	while (a2->call_count < STOP_AUTOMAT_CALL_COUNT)
	{
		...
		if (a1->current_request.id == -1 && (*t1)->len == 0)
		{
			...
			list_beat(t2, a2, sys_time, adress_stack);
			//scanf("%c", &d);
		}
		else if (a2->current_request.id == -1 && (*t2)->len == 0)
		{
			...
			list_beat(t1, a1, sys_time, adress_stack);
		}
		else
		{
...
// Перемещение заявки из очереди в автомат
void time_list_remove_from_turn_to_automat(list_turn_t **t1, automat_t *a1, my_time_t sys_time)
{
	request_t request;
	//output_turn(**t1);
	
	// Удаление заявки из очереди
	time_list_turn_pop(t1, &request);
	(*t1)->len--;
	if ((*t1)->pout == NULL)
		(*t1)->pin = NULL;
...
void time_list_beat(list_turn_t **t1, automat_t *a1, my_time_t sys_time)
{
	//debug_print(DEBUG, "\nSTART BEAT\n");
	//output_automat(*a1);
	if (a1->current_request.id != -1)
	...
	{
		//debug_print(DEBUG, "2. From %d to %d\n", (*t1)->id, a1->id);
		time_list_remove_from_turn_to_automat(t1, a1, sys_time);
	}
	//debug_print(DEBUG, "END BEAT\n\n");
}
